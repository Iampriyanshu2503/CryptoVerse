...
const CIPHER_META: Record<TabId, { title: string; desc: string; badge: string; badgeColor: string }> = {
  ...
  Vigenere:      { title: "Vigenère Cipher",              badge: "Polyalpha",  badgeColor: "bg-blue-500/10 text-blue-400 border-blue-500/20",        desc: "A polyalphabetic cipher using a keyword to apply different shifts." },
    ...
    <div className="p-8 max-w-5xl mx-auto">
      ...
      <div className="mb-7">
        <div className="flex items-center gap-3 mb-1">
          ...
          <span className={`text-[10px] font-semibold uppercase tracking-widest border px-2 py-0.5 rounded-full ${meta.badgeColor}`}>
            {meta.badge}
          </span>
        </div>
      ...
      <div className="flex gap-0.5 bg-gray-900/60 border border-gray-800/80 rounded-xl p-1 mb-7 flex-wrap">
        {TABS.map((tab) => (
          <button
            key={tab}
            onClick={() => { setActiveTab(tab); setResult(null); setBruteForce(null) }}
        ...
        <div>
          ...
          <div className="w-full bg-gray-900/40 border border-gray-800 rounded-xl px-3.5 py-3 text-sm font-mono h-28 overflow-auto">
            {result ? (
              ...
              <span className="text-gray-700">Output will appear here...</span>
            )}
          </div>
        </div>
      </div>
      ...
      <div className="mb-5 space-y-3">
        {activeTab === "Caesar" && (
          <div>
            ...
            <div className="flex items-center gap-3">
              <input
                type="number" min={0} max={25} value={caesarShift}
        ...
        {activeTab === "Hill" && (
          <div>
            ...
            <div className="inline-grid grid-cols-2 gap-1.5">
              {[0, 1].map((r) => [0, 1].map((c) => (
                <input
                  key={`${r}${c}`} type="number" value={hillKey[r][c]}
                  onChange={(e) => {
                    const k = hillKey.map((row: number[]) => [...row])
        ...
        {activeTab === "Rail Fence" && (
          <div>
            ...
            <div className="flex items-center gap-3">
              <input
                type="number" min={2} max={10} value={rails}
        ...
        {activeTab === "Monoalphabetic" && (
          <div>
            <label className="block text-[11px] font-medium text-gray-500 uppercase tracking-wider mb-1.5">
              Substitution Alphabet <span className="text-gray-700 normal-case">(26 chars, maps A→Z)</span>
            </label>
            <input
              value={monoKey}
            ...
            <div className="flex gap-2 mt-2 flex-wrap">
              {["A","B","C","D","E"].map((ch, i) => (
                <span key={ch} className="text-[11px] font-mono text-gray-600">
                  {ch}<span className="text-gray-700">→</span><span className="text-gray-400">{monoKey[i] || "?"}</span>
                </span>
              ))}
      ...
      <div className="flex items-center gap-2 flex-wrap">
        <div className="flex bg-gray-900/60 border border-gray-800 rounded-lg overflow-hidden text-[12px]">
          {(["encrypt", "decrypt"] as const).map((m) => (
            <button
              ...
              className={`px-4 py-1.5 capitalize transition-all ${
        ...
        <button
          onClick={run}
          className="bg-blue-600 hover:bg-blue-500 text-white px-5 py-1.5 rounded-lg text-[13px] font-medium transition-colors flex items-center gap-2"
        >
        ...
        {activeTab === "Caesar" && mode === "decrypt" && (
          <button
            onClick={() => setBruteForce(caesarBruteForce(plaintext))}
  ...
  return (
    <div className="mt-3 inline-grid grid-cols-5 gap-1">
      {chars.slice(0, 25).map((c, i) => (
        <div key={i} className="w-8 h-8 bg-gray-800/80 border border-gray-700/60 rounded-lg flex items-center justify-center text-[12px] font-mono text-gray-300">
          {c}
        </div>
      ))}
    </div>
